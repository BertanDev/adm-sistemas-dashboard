        ...
        return Object.entries(data).map(([chave, valor]) => {
          ...
          return (
            <div key={chave} className="flex py-2 text-gray-600 font-semibold">
              ...
              <span className="text-sm ml-4 pr-2 w-[165px]">R${formatMoney(valor)}</span>
    ...
    return (
        <div>
            <div className="w-[400px] flex justify-between">
                <span className="px-2 bg-red-200 rounded-2xl border-2 border-red-300">Período</span>
                <span className="px-2 bg-red-200 rounded-2xl border-2 border-red-300">Valor a pagar</span>
                <span className="px-2 bg-red-200 rounded-2xl border-2 border-red-300">Porcentagem</span>
            </div>
            {renderData()}
        </div>
    )
