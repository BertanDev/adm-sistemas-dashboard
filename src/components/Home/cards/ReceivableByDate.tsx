...
export async function ReceivableByDate() {
    ...
    const renderData = () => {
        ...
        const total = Object.values(data).reduce((acc: number, curr) => acc + curr, 0);
        ...
        return Object.entries(data).map(([chave, valor]) => {
          ...
          return (
            <div key={chave} className="flex py-2 text-gray-600 font-semibold text-sm">
    ...
    return (
        <div>
            <div className="w-[400px] flex justify-between">
                <span className="px-2 bg-green-200 rounded-2xl border-2 border-green-300">Período</span>
                <span className="px-2 bg-green-200 rounded-2xl border-2 border-green-300">Valor a receber</span>
                <span className="px-2 bg-green-200 rounded-2xl border-2 border-green-300">Porcentagem</span>
            </div>
            {renderData()}
        </div>
    )
